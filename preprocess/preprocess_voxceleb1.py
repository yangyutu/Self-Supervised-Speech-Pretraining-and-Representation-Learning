# -*- coding: utf-8 -*- #
"""*********************************************************************************************"""
#   FileName     [ preprocess_voxceleb1.py ]
#   Synopsis     [ preprocess text transcripts and audio speech for the voxceleb1 dataset ]
#   Author       [ Yuguang Yang ]
...
def preprocess(args, dim):
    ...
    todo = list(Path(args.data_path).rglob("*.[wW][aA][vV]"))
    print(len(todo), f'audio files found in set')

    if args.name == 'None':
        output_dir = os.path.join(args.output_path, '_'.join(['voxceleb1', str(args.feature_type)+str(dim)]))
    else:
    ...
    for file in tqdm(todo):
        name = str(file).split('\\')[-3] + '-' + str(file).split('\\')[-2] + str(file).split('\\')[-1].split('.')[0]
